...
CTrie& operator=(const CTrie &rhs) {
  ...
  /**
   * Overloaded addition/assignment operator to input a new string into the T
rie.
...
CTrie& operator+=(const std::string& word) {
  ...
  /**
   * Overloaded carat operator to check whether a given word exists in the ob
ject.
...
bool operator==(const CTrie& rhs) const {
   ...
   *   e.g. if we have a CTrie with three words 'johns', 'hopkins', 'universi
ty'
